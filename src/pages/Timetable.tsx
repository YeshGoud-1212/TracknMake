  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h1 className="text-xl font-bold text-foreground">Daily Timetable</h1>
      </div>
      <p className="text-sm text-muted-foreground mb-6">Track your discipline daily</p>
      ...
      <div className="flex flex-col gap-2">
        {dayData.tasks.map((task) => (
          <div
            key={task.id}
            className={`flex items-center gap-3 px-4 py-3 bg-card border border-border rounded transition-all duration-200 hover:border-primary/30 group ${task.completed ? "opacity-60" : ""}`}
          >
            {editingId === task.id ? (
              /* Edit mode */
              <div className="flex items-center gap-2 flex-1">
                <input
                  ...
                  className="px-2 py-1 bg-background border border-border rounded text-xs text-foreground w-24"
                />
                <input
                  type="text"
                  value={editLabel}
                ...
                />
                <button onClick={handleEdit} className="p-1 text-success hover:text-success/80"><Check className="w-4 h-4" /></button>
                <button onClick={() => setEditingId(null)} className="p-1 text-muted-foreground hover:text-foreground"><X className="w-4 h-4" /></button>
              </div>
            ) : (
              /* View mode */
              <>
                <button onClick={() => handleToggle(task.id)} className="flex items-center gap-3 flex-1 text-left">
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => startEdit(task.id, task.time, task.label)} className="p-1 text-muted-foreground hover:text-foreground"><Pencil className="w-3.5 h-3.5" /></button>
                  <button onClick={() => handleDelete(task.id)} className="p-1 text-muted-foreground hover:text-destructive"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              </>
            )}
          </div>
      ...
      {showAddForm ? (
        <div className="mt-4 flex gap-2">
          <input type="time" value={newTime} onChange={(e) => setNewTime(e.target.value)} className="px-3 py-2 bg-card border border-border rounded text-sm text-foreground" />
          <input type="text" placeholder="Task name" value={newLabel} onChange={(e) => setNewLabel(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleAdd()} className="flex-1 px-3 py-2 bg-card border border-border rounded text-sm text-foreground placeholder:text-muted-foreground" />
          <button onClick={handleAdd} className="px-4 py-2 bg-primary text-primary-foreground rounded text-sm font-medium">Add</button>
        </div>
      ) : (
        <button onClick={() => setShowAddForm(true)} className="mt-4 w-full py-3 border border-dashed border-border rounded text-sm text-muted-foreground hover:text-foreground hover:border-primary/30 transition-all flex items-center justify-center gap-2">
